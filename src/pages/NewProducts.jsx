...
import { fetchNewestProducts } from '../redux/slices/AnotherProductSlice';
...
const NewProducts = () => {
  ...
  const fetchFilteredProducts = useCallback(
    debounce(async (filteredParam) => {
      try {
        const res = await axiosInstance.get(`/product/newest?${filteredParam}`);
        setFilterProducts({
          status: 'success',
          item: res.data.newestProducts,
        });
      } catch (err) {
